...
function IsLess (a, b : player) : boolean;
begin
 if a.num<b.num then isless:=true
  else if a.num=b.num then
   begin
    if dot[a.nam].num<dot[b.nam].num then isless:=true
     else if dot[a.nam].num=dot[b.nam].num then
     begin
      if a.nam>b.nam then isless:=true else isless:=false;
     end
    else isless:=false;
   end
  else isless:=false;
