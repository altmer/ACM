...
function sieg(func:real;Y:integer): integer;
begin
if (y<0) and(func<0) then sieg:=-1 else sieg:= 1;
end;
...
begin
 ...
 c1x := coss (L1, O, T, C);
 c2x := coss (L2, O, T, C);
 s1x := sieg(c1x,L1.y)*sqrt (1 - sqr (c1x));
 s2x := sieg(c2x,L2.y)*sqrt (1 - sqr (c2x));
 ...
 writeln (cax : 0 : 17, ' ', cay : 0 : 17, ' ', cbx : 0 : 17, ' ', cby : 0 : 17);
